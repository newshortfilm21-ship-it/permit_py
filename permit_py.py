...
with st.sidebar.form('Input_Form'):
    ...
    city_corp=st.selectbox('Is your dhaka city corporation North or South',['North','South'])

      

    ward_input=st.number_input('Enter your Ward number',step=1)
    ...
    land_use=st.selectbox('Enter your land use',list(land_use_groups.keys()))




    plot_length=st.number_input('Enter your plot length (m)',min_value=0.0,help='Length in meters (e.g., 20)')
    plot_width=st.number_input('Enter your plot width (m)', min_value=0.0,help='Width in meters (e.g., 30)')
    g_floor_area=st.number_input('Enter your ground floor area (meter square)',min_value=0.0,help='Ground floor area in meter square (e.g., 300)')
    adj_road_width=st.number_input('Enter your adjacent road width (m)',min_value=0.0,help='Road width in meters (e.g., 15)')
    submit=st.form_submit_button('Calculate')

if submit:
   if not all([user_name,ward_input>0,plot_length>0,plot_width>0,g_floor_area>0,adj_road_width>0]):
      st.warning('Please fill all inputs first.')
      st.stop()
